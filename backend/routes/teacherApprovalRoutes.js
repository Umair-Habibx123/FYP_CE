...
router.get('/getApprovalDetailForTeacherWhoApprove/:projectId', async (req, res) => {
    const { projectId } = req.params;
    const { teacherId } = req.query;

    console.log(`Fetching approval details for projectId: ${projectId}, teacherId: ${teacherId}`);
    ...
    try {
        ...
        const approval = project.approvals.find(approval => approval.teacherId === teacherId);
...
router.get("/getAllProjectsWithStatusForTeachers", async (req, res) => {
    try {
        ...
        approvals.forEach((approval) => {
            const universityApproval = approval.approvals.find(app => app.university === university);
...
router.get("/checkAnyTeacherApproveForYourUni/:projectId/:university", async (req, res) => {
    try {
        ...
        try {
            const userResponse = await fetch(`${process.env.VITE_REACT_APP_BACKEND_BASEURL}/api/other-user-profile?email=${universityApproval.teacherId}`, {
                method: "GET",
                headers: {
            ...
            if (userResponse.ok) {
                ...
                console.error("Failed to fetch user profile", await userResponse.json());
            }
        } catch (err) {
        ...
        console.error("Error fetching teacher approval with user details:", error);
        res.status(500).json({ error: "Internal Server Error" });
    }
});
...
router.get("/getProjectsApprovedByYourUniWithDetails/:university", async (req, res) => {
    try {
        ...
        if (!teacherApprovals || teacherApprovals.length === 0) {
            return res.status(200).json({ projects: [], message: "No approved projects found for this university" });
        }
        ...
        const projectsWithApprovalDetails = projects.map(project => {
            ...
            const approvalDoc = teacherApprovals.find(a => a._id === project._id.toString());


            const universityApproval = approvalDoc.approvals.find(a =>
                a.university === university && a.status === "approved"
            );
...
router.get("/getUniversitiesStatusAcrossProject", async (req, res) => {
    try {
        ...
        if (!project) {
            return res.status(404).json({ message: "Project not found", status: "not found" });
        }
...
router.get('/approval-status/:id/:university', async (req, res) => {
    try {
        ...
        if (!teacherApproval) {
            return res.status(404).json({ message: 'Teacher approval record not found' });
        }
        ...
        if (universityApprovals.length === 0) {
            return res.status(404).json({ message: 'No approvals found for the specified university' });
        }
...
router.get('/getUniversityApprovals', async (req, res) => {
    try {
      ...
      if (!projectId || !university) {
        return res.status(400).json({ error: 'Project ID and university name are required' });
      }
