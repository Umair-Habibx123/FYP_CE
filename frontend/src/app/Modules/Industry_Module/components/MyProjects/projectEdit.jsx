...
import { PROJECT_TYPES, SKILL_SUGGESTIONS, DIFFICULTY_LEVEL } from '../../../../../constants/constants.js';
...
const ProjectEdit = ({ project, onCancel }) => {
        ...
        fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/fetchProjectDetailById/${id}`)
            ...
            .then((data) => {
                setProject({
                    ...data,
                    duration: {
                        startDate: data.duration?.startDate ? data.duration.startDate.slice(0, 10) : "",
                        endDate: data.duration?.endDate ? data.duration.endDate.slice(0, 10) : "",
                    },
                });
            })
    ...
    const handleSaveChanges = async () => {
        ...
        if (!editedProject.title || !editedProject.projectType || !editedProject.difficultyLevel || !editedProject.industryName) {
            setWarningMessage("Please fill all required fields (Title, Project Type, Difficulty Level, and Industry Name).");
            setIsModalOpen(false);
            setIsWarningOpen(true);
            return; // Stop further execution
        }
        // Check if maxGroups is valid for group projects
        if (editedProject?.selection === "Group" && (!editedProject?.maxGroups && editedProject?.maxGroups < 1)) {
            setWarningMessage("Please specify a valid number of groups (must be at least 1).");
            setIsModalOpen(false);
            setIsWarningOpen(true);
            return; // Stop further execution
        }
        ...
        if (!editedProject?.duration?.startDate || !editedProject?.duration?.endDate) {
        ...
        if (new Date(editedProject?.duration?.startDate) >= new Date(editedProject?.duration?.endDate)) {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/updateProject/${project._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        {skillInput && filteredSuggestions.length > 0 && (
                            <div className={`absolute z-10 mt-2 w-full rounded-lg shadow-lg max-h-60 overflow-y-auto ${theme === "dark"
                    ...
                    {editedProject?.attachments && editedProject.attachments.length > 0 ? (
                        <ul>
                            {editedProject.attachments.map((attachment) => (
                                <li key={attachment?._id?.$oid || attachment?.fileUrl} className="flex justify-between items-center p-2 border-b last:border-none">
                                    <a
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-gray-500 text-sm">
                                            ...
                                            {attachment?.uploadedAt?.$date?.$numberLong
                                                ? new Date(parseInt(attachment.uploadedAt.$date.$numberLong)).toLocaleString()
                                                : "N/A"}
                                        </span>
                                        {deletingFile === attachment.fileUrl ? (
                                            ...
                                            <button
                                                onClick={() => handleDeleteAttachment(attachment.fileUrl)}
                    ...
                    {files.length > 0 && (
                        <ul className="space-y-2">
                            {files.map((file, index) => (
                                <li key={index} className="flex justify-between items-center p-2 rounded-lg bg-gray-100 dark:bg-gray-700">
                                    <span>{file.name}</span>
                                    <button
