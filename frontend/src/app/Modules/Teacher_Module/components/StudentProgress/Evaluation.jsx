...
const Evaluation = () => {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                ...
                // 1. Fetch project details (basic project info)
                ...
                // 2. Fetch teacher supervision status (only if user data is available)
                const supervisionPromise = projectId && user?.email && user?.teacherDetails?.university
                    ? axios.post(
                        ...
                        .then(response => {
                            setCanAccess(response.data.status === "approved");
                            return response.data;
                        })
                        .catch(err => {
                            console.error("Error fetching teacher supervision status:", err);
                            if (err.response?.status === 404) {
                                setCanAccess(false);
                            } else {
                                setError("Failed to load supervision status");
                            }
                            return null;
                        })
                    : Promise.resolve(null);

                // Wait for project and supervision status before fetching submissions
                const [projectData, supervisionData] = await Promise.all([
                    projectPromise,
                    supervisionPromise
                ]);
        ...
        fetchAllData();
    }, [projectId, selectionId, user, canAccess]);
    ...
    if (loading || isAuthLoading || !project) {
        return <Loading />;
    }
