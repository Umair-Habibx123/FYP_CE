...
const StudentProgress = ({ theme }) => {
    ...
    const fetchProjectsSupervisedByYou = async () => {
        if (user && user.email) {
            setIsProjectsLoading(true);
            try {
                const response = await axios.get(
                    `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/fetchProjectsWithStudentsByUniversity`,
                    {
                        params: {
                ...
                // Filter projects where supervisionStatus is "supervised_by_you"
                const supervisedProjects = response.data.projects.filter(
                    project => project.supervisionStatus === "supervised_by_you"
                );

                // Sort projects
                const sortedProjects = supervisedProjects.sort((a, b) => {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {currentProjects.length === 0 ? (
                        ...
                        currentProjects.map((project) => {
                            ...
                            const selections = project.selectionsWithStudents || [];
                            const firstSelection = selections[0] || {};
                            const groupMembers = firstSelection.groupMembers || [];
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() => navigate(`/studentProgressStd/${project._id}_evaluation`)}
                                >
                                        ...
                                        {groupMembers.length > 0 && (
                                            <div className="mt-2">
                                                <div className="flex justify-end -space-x-3 mt-2">
                                                    {project.selectionsWithStudents[0].groupMembers.map((member, index) => (
                                                        <div
                                                            key={index}
                                                            className="relative w-8 h-8 rounded-full border-2 border-white"
                                                            style={{ zIndex: project.selectionsWithStudents[0].groupMembers.length - index }}
                                                        >
                                                            {member.profilePic ? (
                                                                <img
