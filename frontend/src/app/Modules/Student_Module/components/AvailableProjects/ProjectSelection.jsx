...
import { CheckCircle, Users, PlusCircle, Loader2, Download } from "lucide-react";
...
const ProjectSelection = () => {
        ...
        const fetchAllData = async () => {
            try {
                ...
                const projectResponse = await fetch(
                    `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/fetchProjectDetailById/${id}`
                );
                ...
                // 2. Check user in project (if user exists)
                if (user?.email) {
                    ...
                    setIsUserInProject(userInProjectData.message === "User is in the project");
                }

                // 3. Fetch approval status (if user exists with student details)
                if (user?.studentDetails?.university) {
                    const approvalResponse = await fetch(
                        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/approval-status/${id}/${user.studentDetails.university}`
                    );
    ...
    const handleJoinExistingGroup = async (selectionId) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/JoinExistingGroupforProject`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                setIsUserInProject(true); // Disable buttons after joining
                return { success: true }; // Indicate success
            } else {
                toast.error(data.message);
                return { success: false, error: data.message }; // Pass error message
            }
        } catch (error) {
            toast.error('Failed to join existing group');
            console.error('Error joining existing group:', error);
            return { success: false, error: 'Failed to join existing group' }; // Pass error message
        } finally {
    ...
    const handleSelectProjectAsNewGroup = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/SelectProjectAsNewGroup`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    projectId: id,
                    userEmail: user.email, // Assuming user object contains email
            ...
            if (response.ok) {
                toast.success(data.message);
                setIsUserInProject(true); // Disable buttons after creating new group
            } else {
                toast.error(data.message);
            }
        } catch (error) {
    ...
    if (!project || invalidProjectSearch || status === "not_found" || user.role !== "student") {
        return (
            <div className={`-mt-[70px] md:-mt-[90px] min-h-screen flex items-center justify-center p-6 ${theme === "dark" ? "bg-gray-900 text-gray-300" : "bg-gray-100 text-gray-900"}`}>
...
${theme === "dark" ? "bg-gray-900 text-gray-100 border-gray-700" : "bg-white text-gray-900 border-gray-300"}`}
            ...
            >
                <motion.div className="space-y-8">
                    ...
                    {isUserInProject && (
                        <div className={`mb-4 p-4 rounded-lg flex items-center space-x-3 ${theme === "dark" ? "bg-green-900 text-green-300" : "bg-green-100 text-green-900"}`}>
                            <CheckCircle className="w-6 h-6" />
                            <span>You have already selected this project and are part of a selection group.</span>
                        </div>
                    )}
